...
import com.example.Util.retailerUtil;
...
public class RetailerServiceImpl implements RetailerService {
	...
	public Rewards getRetailerByCustomerId(Long customerId) {

		Timestamp lastMonthTimestamp = getDateBasedOnOffSetDays(retailerUtil.daysInMonths);
		Timestamp lastSecondMonthTimestamp = getDateBasedOnOffSetDays(2 * retailerUtil.daysInMonths);
		Timestamp lastThirdMonthTimestamp = getDateBasedOnOffSetDays(3 * retailerUtil.daysInMonths);
	...
	private Long calculateRewards(Transaction t) {
		if (t.getTransactionAmount() > retailerUtil.firstRewardLimit
				&& t.getTransactionAmount() <= retailerUtil.secondRewardLimit) {
			return Math.round(t.getTransactionAmount() - retailerUtil.firstRewardLimit);
		} else if (t.getTransactionAmount() > retailerUtil.secondRewardLimit) {
			return Math.round(t.getTransactionAmount() - retailerUtil.secondRewardLimit) * 2
					+ (retailerUtil.secondRewardLimit - retailerUtil.firstRewardLimit);
		} else
			return 0l;
